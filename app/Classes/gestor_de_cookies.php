...
class Gestor_de_cookies{
	const ACTIVAR_PARCHE_TEMPORAL=false;
		...
		if($tipo_usuario==LOGIN_DE_USUARIO_EXTERNO) {
			$value=self::pre_cifrar($id_usuario);
			$authcode=Authcode::USUARIO_EXTERNO;
		}
		if($tipo_usuario==LOGIN_DE_USUARIO_INTERNO) {
			$authcode=Authcode::USUARIO_INTERNO;
			$value=$id_usuario;
		}
	...
	public static function get($value,$tipo_usuario){
		...
		if($tipo_usuario==LOGIN_DE_USUARIO_EXTERNO) $authcode=Authcode::USUARIO_EXTERNO;
		if($tipo_usuario==LOGIN_DE_USUARIO_INTERNO) $authcode=Authcode::USUARIO_INTERNO;
