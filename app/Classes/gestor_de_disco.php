...
//        var_dump(PATH_PUBLIC."PHPExcel/Classes/PHPExcel/IOFactory.php");
//        require_once PATH_PUBLIC."PHPExcel/Classes/PHPExcel/IOFactory.php";
        ...
        try {
            ...
            $objeto->getProperties()
                    ...
                    ->setTitle("Documento Excel de" . APP_NAME)
                    ->setSubject("Documento Excel de" . APP_NAME)
                    ->setDescription("Este archivo ha sido exportado desde " . APP_NAME);
            $objeto->getActiveSheet()->setTitle(APP_NAME);
                    ...
                    if ($array[$columna] !== null AND $el_formato == self::FORMATO_FECHA) {
                        if (!($fecha_temp = DateTime::createFromFormat('d/m/Y', $array[$columna]))) {
                            if (self::ACTIVAR_DEBUG)
                                developer_log('El formato de la fecha no es correcto.');
                            return false;
                        }
                        $el_valor = PHPExcel_Shared_Date::PHPToExcel($fecha_temp);
                    } else
                        $el_valor = $array[$columna];
                    $la_celda = $col . $numero_fila;

                    if ($el_valor == null)
                        $el_tipo = self::TIPO_TEXTO;# Para que no ponga un 0 en los tipos numericos que son null
...
//        require_once PATH_PUBLIC."PHPExcel/Classes/PHPExcel/IOFactory.php";
        ...
        try {
            ...
            if (!$objeto = PHPExcel_IOFactory::load($archivo))
                return false;
            if (!$worksheet = $objeto->getActiveSheet())
                return false;
    ...
    public function comprimir_zip($path, $filename, $archivos) {
        try {
            $zip = new ZipArchive();
            if ($zip->open($this->path_home . $path . $filename, ZipArchive::CREATE) !== TRUE)
                return false;
...
//                developer_log($this->path_home.$path.$archivo);
    ...
    public function generar_pdf($resultado, $filename) {

        $mpdf = new \Mpdf\Mpdf(['tempDir' => PATH_PUBLIC_FOLDER,
            "defaultCssFile"=>__DIR__."/../Http/views/css_pdf.css"]);
//        $mpdf = new \Mpdf\Mpdf(['tempDir' => PATH_PUBLIC_FOLDER]);
        ...
        foreach ($headers as $key => $value) {
            $th = $vista->createElement("th", $value);
            $tr->appendChild($th);
        }
        ...
        foreach ($resultado as $row) {
            ...
            foreach ($row as $key => $value) {
                $td = $vista->createElement("td", $value);
                $tr->appendChild($td);
            }
        }
        $table->appendChild($tr);
        $vista->appendChild($table);
...
//        $mpdf->WriteHTML($css,1);
        ...
        $mpdf->WriteHTML('<div>Fecha:'.$fecha->format("Y-m-d H:i:s").'</div>',2);
        $mpdf->WriteHTML('<h1>Resultado:</h1>',2);
