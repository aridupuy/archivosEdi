...
<?php
include PATH_VENDOR_HANDLE_ERROR;
...
//            $DB->debug=99;
//            define('ADODB_ERROR_LOG_TYPE',1);  
//            define('ADODB_ERROR_LOG_DEST','/var/log/adodb/errors.log');  
    ...
    public static function execute_select($sql,$variables=false,$limit=-1,$offset=-1){
        ...
        try{
            ...
            if($limit==0)
                $limit=MAXIMO_REGISTROS_POR_CONSULTA_CONSOLA;
...
//        developer_log($variables);
    ...
    public static function select_order($variables=false,$campo,$ascdesc){
        $filtros=self::preparar_filtros($variables);
//        developer_log($variables);
        $tabla = static::$prefijo_tabla.strtolower((get_called_class())); 
        $id_tabla = strtolower(static::$id_tabla); 
    ...
    public static function select_limit_offset($variables=false,$offset=0,$limit=-1){
        $filtros=self::preparar_filtros($variables);
//        developer_log($variables);
        $tabla = static::$prefijo_tabla.strtolower((get_called_class())); 
        $id_tabla = strtolower(static::$id_tabla); 
        ...
        $result=self::execute_select($sql,$variables,$limit,$offset);   
        if($result) { return $result; }
        return false;
    }
